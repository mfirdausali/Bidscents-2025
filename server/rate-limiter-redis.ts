import rateLimit, { Options, Store } from 'express-rate-limit';
import { Request, Response } from 'express';
import { auditSecurity } from './audit-logger';
import { getRateLimitStore, RedisRateLimitStore } from './redis-stores';
import { getRedisClient } from './redis-client';

/**
 * Redis-backed rate limiting for BidScents with in-memory fallback
 * Provides distributed rate limiting across multiple server instances
 */

// Fallback to in-memory store when Redis is unavailable
let useMemoryFallback = false;
let rateLimitStore: RedisRateLimitStore;

// Initialize rate limit store
function initializeRateLimitStore() {
  try {
    rateLimitStore = getRateLimitStore();
    const redis = getRedisClient();
    
    // Monitor Redis connection
    redis.on('error', () => {
      console.error('[RateLimit] Redis error, switching to in-memory fallback');
      useMemoryFallback = true;
    });
    
    redis.on('connect', () => {
      console.log('[RateLimit] Redis connected, switching back from fallback');
      useMemoryFallback = false;
    });
    
    // Check if Redis is ready
    if (!redis.isReady()) {
      console.warn('[RateLimit] Redis not ready, using in-memory fallback');
      useMemoryFallback = true;
    }
  } catch (error) {
    console.error('[RateLimit] Failed to initialize Redis store, using in-memory fallback:', error);
    useMemoryFallback = true;
  }
}

// Initialize on module load
initializeRateLimitStore();

/**
 * Create Redis store for express-rate-limit with fallback
 */
function createRedisStore(windowMs: number): Store | undefined {
  if (useMemoryFallback) {
    // Return undefined to use default in-memory store
    return undefined;
  }

  return {
    increment: async function(key: string): Promise<{ totalHits: number; resetTime?: Date }> {
      try {
        const result = await rateLimitStore.increment(key, windowMs);
        if (result) {
          return {
            totalHits: result.count,
            resetTime: new Date(result.resetAt)
          };
        }
        // If Redis fails, switch to fallback
        useMemoryFallback = true;
        throw new Error('Redis operation failed');
      } catch (error) {
        console.error('[RateLimit] Redis increment failed:', error);
        // Re-throw to trigger fallback
        throw error;
      }
    },
    
    decrement: async function(key: string): Promise<void> {
      // Not typically used in rate limiting, but implemented for completeness
      console.log('[RateLimit] Decrement called for key:', key);
    },
    
    resetKey: async function(key: string): Promise<void> {
      try {
        await rateLimitStore.reset(key);
      } catch (error) {
        console.error('[RateLimit] Redis reset failed:', error);
      }
    }
  };
}

// Helper to create key generator with IP + user agent fingerprinting
const createKeyGenerator = (includeUserId = false) => {
  return (req: Request & { userId?: string }) => {
    const baseKey = req.ip + ':' + req.get('user-agent');
    if (includeUserId && req.userId) {
      return baseKey + ':' + req.userId;
    }
    return baseKey;
  };
};

// Progressive delay handler for repeated violations
const progressiveDelayHandler = async (req: Request, res: Response, next: any, options: Options) => {
  const retryAfter = Math.round((options.windowMs || 60000) / 1000);
  
  // Audit rate limit violation
  await auditSecurity.rateLimitExceeded(req, req.path).catch(err => {
    console.error('Failed to audit rate limit violation:', err);
  });
  
  res.status(options.statusCode || 429).json({
    error: options.message || 'Too many requests',
    retryAfter,
    resetTime: new Date(Date.now() + (options.windowMs || 60000)).toISOString(),
    storage: useMemoryFallback ? 'memory' : 'redis'
  });
};

// === AUTHENTICATION RATE LIMITERS ===

// Strict rate limit for authentication endpoints (login, register)
export const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 5, // Limit each IP to 5 requests per windowMs
  message: 'Too many authentication attempts, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(false),
  handler: progressiveDelayHandler,
  store: createRedisStore(15 * 60 * 1000)
});

// Moderate rate limit for password reset and email verification
export const passwordResetLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 3, // Limit each IP to 3 password reset requests per hour
  message: 'Too many password reset attempts, please try again later',
  skipSuccessfulRequests: true, // Only count failed attempts
  keyGenerator: createKeyGenerator(false),
  handler: progressiveDelayHandler,
  store: createRedisStore(60 * 60 * 1000)
});

// Strict rate limit for user enumeration endpoints
export const userLookupLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 10, // Only 10 lookups per 15 minutes
  message: 'Too many lookup requests, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(15 * 60 * 1000)
});

// === RESOURCE CREATION RATE LIMITERS ===

// Rate limiter for creating products, auctions
export const resourceCreationLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 20, // 20 creations per hour per user
  message: 'Too many resource creation attempts, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: true,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(60 * 60 * 1000)
});

// Rate limiter for reviews and ratings
export const reviewLimiter = rateLimit({
  windowMs: 24 * 60 * 60 * 1000, // 24 hours
  max: 10, // 10 reviews per day per user
  message: 'Too many review submissions, please try again tomorrow',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(24 * 60 * 60 * 1000)
});

// === PAYMENT RATE LIMITERS ===

// Strict rate limiter for payment endpoints
export const paymentLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 10, // 10 payment attempts per hour
  message: 'Too many payment attempts, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  handler: progressiveDelayHandler,
  store: createRedisStore(60 * 60 * 1000)
});

// Webhook rate limiter (more permissive for legitimate webhook calls)
export const webhookLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 30, // 30 webhook calls per minute
  message: 'Too many webhook requests',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  store: createRedisStore(60 * 1000)
});

// === FILE UPLOAD RATE LIMITERS ===

// Strict rate limiter for file uploads
export const fileUploadLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 50, // 50 uploads per hour per user
  message: 'Too many file uploads, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(60 * 60 * 1000)
});

// Very strict limiter for profile image uploads
export const profileImageLimiter = rateLimit({
  windowMs: 24 * 60 * 60 * 1000, // 24 hours
  max: 5, // 5 profile image updates per day
  message: 'Too many profile image updates, please try again tomorrow',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(24 * 60 * 60 * 1000)
});

// === MESSAGING RATE LIMITERS ===

// Rate limiter for sending messages
export const messagingLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 30, // 30 messages per minute per user
  message: 'Too many messages sent, please slow down',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(60 * 1000)
});

// Rate limiter for message file uploads
export const messageFileLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 20, // 20 file messages per hour
  message: 'Too many file uploads in messages, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(60 * 60 * 1000)
});

// === GENERAL API RATE LIMITERS ===

// General API rate limit for read operations
export const apiLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 100, // Limit each IP to 100 requests per windowMs
  message: 'Too many requests, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  store: createRedisStore(15 * 60 * 1000)
});

// More generous limiter for public read endpoints
export const publicReadLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 200, // 200 requests per 15 minutes
  message: 'Too many requests, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  store: createRedisStore(15 * 60 * 1000)
});

// Authenticated read operations limiter
export const authenticatedReadLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 300, // 300 requests per 15 minutes for authenticated users
  message: 'Too many requests, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(15 * 60 * 1000)
});

// === ADMIN RATE LIMITERS ===

// Admin endpoints rate limit (more permissive)
export const adminLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 200, // Higher limit for admin operations
  message: 'Too many admin requests, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(15 * 60 * 1000)
});

// === SPECIALIZED RATE LIMITERS ===

// Auction bidding rate limiter
export const biddingLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 20, // 20 bids per minute (to allow competitive bidding)
  message: 'Too many bids, please slow down',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(60 * 1000)
});

// Search endpoint rate limiter
export const searchLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 30, // 30 searches per minute
  message: 'Too many search requests, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  store: createRedisStore(60 * 1000)
});

// Boost operations rate limiter
export const boostLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 10, // 10 boost operations per hour
  message: 'Too many boost operations, please try again later',
  standardHeaders: true,
  legacyHeaders: false,
  skipSuccessfulRequests: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(60 * 60 * 1000)
});

// Cart operations rate limiter
export const cartLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 60, // 60 cart operations per minute
  message: 'Too many cart operations, please slow down',
  standardHeaders: true,
  legacyHeaders: false,
  keyGenerator: createKeyGenerator(true),
  store: createRedisStore(60 * 1000)
});

// Social preview generation limiter
export const socialPreviewLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 100, // 100 preview generations per hour
  message: 'Too many preview requests',
  standardHeaders: true,
  legacyHeaders: false,
  store: createRedisStore(60 * 60 * 1000)
});

/**
 * Get rate limit storage status
 */
export function getRateLimitStorageStatus(): { type: string; ready: boolean } {
  return {
    type: useMemoryFallback ? 'memory' : 'redis',
    ready: useMemoryFallback ? true : getRedisClient().isReady()
  };
}

/**
 * Force switch to memory fallback (for testing)
 */
export function forceMemoryFallback(enable: boolean): void {
  useMemoryFallback = enable;
  console.log(`[RateLimit] Forced memory fallback: ${enable}`);
}