...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/products/:id/images", async (req, res, next) => {
    try {
      ...
      console.log(`GET /api/products/${productId}/images`);
      
      // Get images directly from Supabase
      ...
      console.log(`Found ${images?.length || 0} images for product ${productId}`);
      res.json(images || []);
    } catch (error) {
  ...
  // Simplified endpoint to handle image upload in a single step 
  app.post("/api/products/:id/images", upload.single('image'), async (req, res, next) => {
    try {
      console.log(`POST /api/products/:id/images - Starting image upload process`);
      ...
      if (req.isAuthenticated() && req.user) {
        ...
        sellerId = req.user.id;
        console.log(`User authenticated via session, seller ID: ${sellerId}`);
      } else if (req.query.sellerId) {
        // If not via session, check if sellerId was provided in query
        sellerId = parseInt(req.query.sellerId as string);
        console.log(`Using seller ID from query: ${sellerId}`);
      ...
      const imageOrder = req.body.imageOrder ? parseInt(req.body.imageOrder) : 0;
      console.log(`Using image order: ${imageOrder}`);
      
      // STEP 1: Generate a new UUID for the image
      const uuid = objectStorage.generateImageId();
      console.log(`Generated image UUID: ${uuid}`);
      
      // STEP 2: Add record in product_images table
      const { data: imageRecord, error: insertError } = await supabase
        .from('product_images')
        .insert({
          product_id: productId,
          image_url: uuid,  // Store just the UUID in the database
          image_order: imageOrder,
          image_name: req.file.originalname || 'unnamed'
        })
        .select()
        .single();
      ...
      if (insertError) {
        ...
        return res.status(500).json({ message: "Failed to create image record in database" });
      }
      
      console.log(`Image record created with ID: ${imageRecord.id}`);
      
      // STEP 3: Upload image to object storage
      console.log(`Uploading file to object storage with UUID: ${uuid}`);
      const uploadResult = await objectStorage.uploadProductImage(
        req.file.buffer,
        uuid,  // Use UUID as the storage key
        req.file.mimetype
      );
      ...
      if (!uploadResult.success) {
        ...
        console.error('Upload to object storage failed, removing database record');
        await supabase
          .from('product_images')
          .delete()
      ...
      // STEP 4: Return success with image details
      res.status(201).json({
        id: imageRecord.id,
        productId: imageRecord.product_id,
        imageUrl: imageRecord.image_url,
        imageOrder: imageRecord.image_order,
