...
console.log(
  `Initialized message files storage client with bucket: ${messageBucketId}`,
);
...
export async function uploadProductImage(
  ...
  try {
    ...
    console.log(
      `Successfully uploaded image ${imageId} to Replit Object Storage`,
    );
...
export async function deleteProductImage(imageId: string): Promise<boolean> {
  try {
    ...
    console.log(
      `Successfully deleted image ${imageId} from Replit Object Storage`,
    );
    return true;
  } catch (error) {
...
export async function getImageFromStorage(
  ...
  try {
    ...
    if (!result.ok || !result.value) {
      console.error(
        "Error retrieving image from Replit Object Storage:",
        result.error,
      );
      return null;
    }
...
export async function uploadProfileImage(
  ...
  try {
    ...
    console.log(
      `Successfully uploaded profile image ${imageId} for user ${userId}`,
    );
    return { url: imageId, success: true };
  } catch (error) {
...
export async function uploadCoverPhoto(
  ...
  try {
    ...
    console.log(
      `Successfully uploaded cover photo ${imageId} for user ${userId}`,
    );
    return { url: imageId, success: true };
  } catch (error) {
...
export async function uploadMessageFile(
  ...
  try {
    ...
    const result = await messageFileStorageClient.uploadFromBytes(
      fileId,
      fileBuffer,
    );
 ...
 */
export async function getMessageFileFromStorage(
  fileId: string,
): Promise<Buffer | null> {
  console.log(
    `🔍 Attempting to retrieve message file: "${fileId}" from storage`,
  );
  ...
  try {
    ...
    if (!result.ok || !result.value) {
      console.error(
        `❌ Error retrieving message file ${fileId}:`,
        result.error,
      );
      return null;
    }
    ...
    // Log success
    console.log(
      `✅ Successfully retrieved message file from storage: ${fileId}`,
    );
