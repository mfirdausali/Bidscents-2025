...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Product voting system - upvote
  app.post("/api/products/:id/upvote", async (req, res, next) => {
    try {
      // Check authentication
      if (!req.isAuthenticated()) {
      ...
      if (getError) {
        console.error('Error fetching product votes:', getError);
      ...
      const { data: updatedProduct, error: updateError } = await supabase
        .from('products')
        .update({ votes: (product.votes || 0) + 1 })
        .eq('id', productId)
        .select()
        .single();
      ...
      return res.status(200).json({ votes: updatedProduct.votes });
    } catch (err) {
      console.error('Error in upvote endpoint:', err);
      return res.status(500).json({ message: 'Server error' });
    }
  });
  
  // Product voting system - downvote
  app.post("/api/products/:id/downvote", async (req, res, next) => {
    try {
      // Check authentication
      if (!req.isAuthenticated()) {
        return res.status(401).json({ message: "You must be logged in to vote" });
      ...
      const newVotes = Math.max(0, (product.votes || 0) - 1);
      ...
      console.error('Error in downvote endpoint:', err);
      return res.status(500).json({ message: 'Server error' });
    }
  });
