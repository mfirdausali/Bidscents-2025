          ...
          try {
            ...
            for (let i = 0; i < productIds.length; i++) {
              ...
              try {
                ...
                // Check if a product payment record already exists for this bill ID and product ID
                try {
                  ...
                  if (existingRecords && existingRecords.length > 0) {
                    console.log(`⚠️ Payment record already exists for product #${productId} with bill_id ${effectiveBillId}, skipping creation`);
                    createdPayments.push(existingRecords[0]);
                    
                    // Still update the product to be featured
                    await updateProductFeatureStatus(productId, payment);
                    continue;
                  }
                } catch (checkErr) {
                  console.error(`❌ Error checking for existing payment:`, checkErr);
                }
                ...
                const insertData = {
                  ...
                  status: 'paid',
                  paid_at: paidDate || new Date(),
                  // Don't include created_at as it's auto-generated
                ...
                // Create a type-corrected insert data object to ensure all fields have appropriate data types
                const typeFixedData = {
                  ...insertData,
                  order_id: uniqueOrderId,  // Ensure it's a string in UUID format
                  product_id: Number(productId),  // Ensure it's a number
                  user_id: typeof userId === 'string' ? parseInt(userId, 10) : userId,  // Ensure it's a number
                  amount: Number(Math.floor(payment.amount / productIds.length)),  // Ensure it's a number
                  paid_at: new Date(paidDate || new Date()).toISOString(),  // Ensure ISO string format
                };
                
                console.log(`🔬 DIAGNOSTIC: Type-corrected insert data:`, JSON.stringify(typeFixedData, null, 2));
                
                // Try the Supabase insert operation with type-corrected data
                try {
                  console.log(`🔬 DIAGNOSTIC: Attempting Supabase insert operation with type-corrected data`);
                  const { data: newPayment, error } = await supabase
                    .from('payments')
                    .insert(typeFixedData)
                    .select()
                    .single();
                  ...
                  if (error) {
                    ...
                    // Continue instead of throwing to process other products
                    console.error(`⚠️ Skipping product #${productId} due to error - continuing with other products`);
                    continue;
                  }
                  ...
                  if (newPayment) {
                    ...
                    // Update the product featured status using our helper function
                    await updateProductFeatureStatus(productId, payment);
                  }
                } catch (err) {
